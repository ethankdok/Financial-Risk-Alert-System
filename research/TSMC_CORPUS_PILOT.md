...
- `scripts/build_tsmc_corpus.py` 下載 PDF、抽取文字、驗證官方 HTTPS 網域、紀錄 SHA256／來源網頁／文件期間，產生本機 CSV、與 STRUX 類似的 Parquet 及逐期狀態表。
...
原始語料：本機 `data/tsmc_corpus/tsmc_quarterly_text.parquet`（及 CSV），不提交 GitHub。分析結果：本機 `data/tsmc_corpus/calibration_result.json`。請先確認完整性、PDF 轉文字格式和語言，才能當研究成果使用。部分逐字稿包含翻譯與 Q&A；目前先固定為**完整逐字稿對完整逐字稿**，不得與僅 prepared remarks 的 STRUX 結果直接比較。
