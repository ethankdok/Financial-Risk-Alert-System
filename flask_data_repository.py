...
class SqliteFlaskDataRepository:
    ...
    def create_analysis_record(self, payload: dict[str, Any]) -> int:
        with self._connect() as connection:
            cursor = connection.execute(
                "INSERT INTO analysis_records(query_text,risk_score,raw_score,risk_level,matched_keywords_json,matched_features_json,created_at) "
                "VALUES(?,?,?,?,?,?,?)",
                (
                    payload["query_text"], payload["risk_score"], payload["raw_score"], payload["risk_level"],
