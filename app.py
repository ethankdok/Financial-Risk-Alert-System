...
def ensure_seed_data() -> None:
    """Keep the existing local demo experience without auto-creating cloud demo users.

    Firestore production deployments should receive data through the migration step.
    Set ALLOW_DEMO_SEED_DATA=true explicitly only for a disposable cloud demo project.
    """
...
def analyze_text():
    ...
    for row in repository.list_risk_features(active_only=True):
        feature_keywords = list(row.get("keywords") or [])
        hits = [keyword for keyword in feature_keywords if keyword and keyword in text]
        if hits:
